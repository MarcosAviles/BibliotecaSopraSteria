...
public class EditorialDAO {
    ...
    public static boolean registrar(Editorial edi){
        try {
            String SQL = "INSERT INTO editoriales (nit, nombre, telefono, direccion, email, sitioweb) values (?,?,?,?,?,?)"; // Por cada ? un setString
        ...
        try {
            String SQL = "UPDATE editoriales " +
                "SET " +
                    ...
                    "email=?," +
                    "sitioweb=?" +
                "WHERE nombre=?"; // Por cada ? un setString
            Connection con= Conexion.conectar();
            PreparedStatement st = con.prepareStatement(SQL);
            st.setString(1, edi.getNit());
    ...
    public static boolean eliminar(Editorial edi){
        try {
            String SQL = "DELETE FROM editoriales WHERE nombre=?"; // Por cada ? un setString
            Connection con= Conexion.conectar();
            PreparedStatement st = con.prepareStatement(SQL);
    ...
    public static String getNombre(String nit){
        try {
